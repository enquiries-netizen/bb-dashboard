export default async function handler(req, res) {
  ...
  const PAGE_DIRECTORY = pageDirectory || {
    ...
    P6: 'Capacity & Labour (forward-looking workforce planning). Every visual shares one selected ISO week (selector). Layout: (1) Six KPIs: Available Capacity, Scheduled Hours, Forecast Utilisation, Overallocated Capacity, Unallocated Capacity, Forecast Labour Cost (planned vs budgets Pending when incomplete). (2) Crew Allocation Planner Gantt with People View | Project View toggle; crew-level rows expand to employees; fixed cols Department, Crew/Employee, Available Hours, Scheduled Hours, Utilisation; daily next-4-weeks / weekly next-8-weeks timeline; assignment bars from live fields only (Pending for true forward plan when Schedule not live). (3) Department utilisation heatmap 8-12 weeks with thresholds confirmed by constants (Lori/Ben). (4) Planned vs actual hours chart: Available, Scheduled, Actual Productive, Forecast Demand, Today separator. (5) Action panel from real flags only (overallocation, unfilled demand Pending when no demand sheet, spare capacity vs missing timesheets split). (6) Capacity and labour-cost detail in drill-downs. Distinct metrics: Available = contracted net leave/training/holidays from Capacity Hours Available; Scheduled ≠ Logged; Productive Logged hours separate; Overtime Recorded is a timesheet status, not automatic Timesheet Error. Orange only for genuine overallocation/delivery risk. Data Incomplete uses neutral grey. Never invent hours. Live: Capacity, Buildpass_Labour after fingerprint dedupe. Forward assignment / diaries / budgets: Pending. For week-over-week Capacity totals go to P8. Answer only from live page data.',
