...
export default async function handler(req, res) {
  ...
  const PAGE_DIRECTORY = pageDirectory || {
    ...
    P13: 'Tender Pipeline (Admin). Source: Tenders tab (manual). KPIs: monthly opportunity target vs submissions, submissions this month, conversion rate of decided tenders only, pending decision, win rate by value, pipeline value pending. Table + 6-month submissions vs target trend. Won with blank/zero Final Price falls back to quoted Total (Inc GST) with a data-quality flag. Never invent numbers. Answer only from live P13 page data.'
  };
