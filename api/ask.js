...
export default async function handler(req, res) {
  ...
  const PAGE_DIRECTORY = pageDirectory || {
    ...
    P13: 'Tender Pipeline (Admin). Source: Tenders tab (manual). Dollar figures shown ex GST. KPIs: monthly submitted-value target vs actual, submissions this month, conversion rate of decided tenders vs ALL/ALL conversion target when set, pending decision, win rate by value, pipeline value pending. Table + source badges + search + 6-month submitted value vs target trend. Won with blank/zero Final Price falls back to quoted value (ex GST) with a data-quality flag. Never invent numbers. Answer only from live P13 page data.'
  };
