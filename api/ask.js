export default async function handler(req, res) {
  ...
  const PAGE_DIRECTORY = pageDirectory || {
    ...
    P12: 'Site Diaries (crew-only handoff). Staff-gated (temporary session unlock — Lori/Ben must confirm per-person vs shared before production). Condensed job-day summaries from Buildpass_Site Diaries; full Issues/Blockers and safety text; Photo Links list when URLs exist; Variations description+status only NEVER dollar values. Entries without job # stay. Meli weekly record Pending. Not merged with P11 quality dashboard. No Gantt, no financials. Answer only from live page data when unlocked.'
  };
